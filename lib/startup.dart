...
Future<Map<String, List<String>>> fetchLearnsets() async {
  final learnsetsJson = await rootBundle.loadString('assets/data/learnset.json');
...
Future<Map<String, Item>> fetchItems() async {
  final itemsJson = await rootBundle.loadString('assets/data/items.json');
  final Map<String, Item> items = {};
  final json = jsonDecode(itemsJson) as Map<String, dynamic>;
  json.forEach((key, dynamic value) {
    items[key] = Item.fromJson(value as Map<String, dynamic>);
  });
  return items;
}
...
class Startup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: Future.wait([fetchDex(), fetchAbilities(), fetchLearnsets(), fetchMoves(), fetchItems()]),
      builder: (BuildContext context, AsyncSnapshot<List<dynamic>> snapshot) {
        ...
        if (!snapshot.hasData) {
          ...
          final items = snapshot.data[4] as Map<String, Item>;
          return MultiProvider(
            providers: [
              Provider<Map<String, Pokemon>>(create: (_) => pokedex, lazy: false),
              Provider<Map<String, Ability>>(create: (_) => abilities, lazy: false),
              Provider<Map<String, List<String>>>(create: (_) => learnsets, lazy: false),
              Provider<Map<String, Move>>(create: (_) => moves, lazy: false),
              Provider<Map<String, Item>>(create: (_) => items, lazy: false),
              ChangeNotifierProvider(create: (_) => RoomMessages(), lazy: false),
              ChangeNotifierProvider(create: (_) => GlobalMessages(), lazy: false),
            ],
            child: child,
