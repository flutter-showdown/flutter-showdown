        ...
        child: Column(
          children: [
            ...
            FlatButton(
              ...
              onPressed: () async {
                showDialog<LoginDialog>(context: context, builder: (_) => LoginDialog());
              },
            ),
            if (user.group != ' ')
              const SizedBox(width: 8),
            if (user.group != ' ')
              Text(Groups[user.group]),
            const SizedBox(width: 8),
            if (user.named && !user.registered)
              ElevatedButton(
                child: const Text('Register'),
                onPressed: () {
                  showDialog<RegisterDialog>(context: context, builder: (_) => RegisterDialog(user.name));
                },
              ),
          ],
        ),
